 ...
 *         friend_rec.step8.FiltrateTest
 *         -Dinput=/FriendRec/step7
...
public class FiltrateTest extends Configured implements Tool {
	...
	public static class FiltrateReducer extends Reducer<KeyPair, ValueTuple, RecommendationDB, NullWritable> {
		...
		protected void reduce(
				...
				Reducer<KeyPair, ValueTuple, RecommendationDB, NullWritable>.Context context)
				throws IOException, InterruptedException {
			int count = 0;
			for (ValueTuple value : values) {
				if (count < 5) {
					k.setId(key.getId());
					k.setFriend_id(value.getBook_id());
	...
	public int run(String[] args) throws Exception {
		...
		DBOutputFormat.setOutput(job, "recommendation_friends", "rec_day", "id", "friend_id", "rec_degree");
